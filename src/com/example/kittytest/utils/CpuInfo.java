...
public class CpuInfo {
	...
	public ArrayList<String> getCpuRatioInfo() {
		...
		try {
			...
			if (isInitialStatics) {
				...
				if (traffic == -1) {
					KittyService.bw.write(mDateTime2 + "," + pMemory + ","
	...
	try {
		...
		//初始化统计（首次统计）
		long deltaTraffic = 0;
		float trafficSpeed = 1;
		if (isInitialStatics) {
			initialTraffic = trafficInfo.getTrafficInfo();
			isInitialStatics = false;
			deltaTraffic = trafficInfo.getTrafficInfo();		
		}else{
			deltaTraffic = trafficInfo.getTrafficInfo() - lastestTraffic;
			...
			if (initialTraffic == -1){
				...
				traffic = (lastestTraffic - initialTraffic + 1023) / 1024;
				trafficSpeed = (deltaTraffic + 1023)/1024;
			}
			...
			// whether certain device supports traffic statics or not
//			if (traffic == -1) {
//				KittyService.bw.write(mDateTime2 + Constants.COMMA + pMemory + Constants.COMMA
//						+ percent + Constants.COMMA + fMemory + Constants.COMMA + processCpuRatio
//						+ Constants.COMMA + totalCpuRatio + Constants.COMMA + "本程序或本设备不支持流量统计"
//						+ Constants.COMMA + totalBatt + Constants.COMMA + currentBatt + Constants.COMMA
//						+ temperature + Constants.COMMA + voltage + Constants.LINE_END);
//			} else {
				KittyService.bw.write(mDateTime2 + Constants.COMMA + pMemory + Constants.COMMA
						+ percent + Constants.COMMA + fMemory + Constants.COMMA + processCpuRatio
						+ Constants.COMMA + totalCpuRatio + Constants.COMMA + traffic +
						Constants.COMMA + totalBatt + Constants.COMMA + currentBatt + Constants.COMMA
						+ temperature + Constants.COMMA + voltage + Constants.LINE_END);
			}
//		}
